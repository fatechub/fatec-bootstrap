/*!
 * Fatec Bootstrap Customizer (http://getbootstrap.com/customize/)
...
window.onload = function () { // wait for load in a dumb way because B-0
  ...
  function createGist(configJson) {
    var data = {
      'description': 'Fatec Bootstrap Customizer Config',
      'public': true,
      'files': {
        'config.json': {
          'content': configJson
  ...
  function generateZip(css, js, fonts, config, complete) {
    if (!css && !js) return showError('<strong>Ruh roh!</strong> No Fatec Bootstrap files selected.', new Error('no Fatec Bootstrap'))
  ...
  function generateLESS(lessFilename, lessFileIncludes, vars) {
    ...
    $.each(includedLessFilenames(lessFilename), function(index, filename) {
      ...
      // Custom variables are added after Fatec Bootstrap variables so the custom
  ...
  function generateJavascript() {
    ...
    return {
      'fatec-bootstrap.js': js,
      'fatec-bootstrap.min.js': cw + uglify(js)
    }
  }
  ...
  if (!window.URL && navigator.userAgent.toLowerCase().indexOf('safari') != -1) {
    ...
    showCallout('Looks like your current browser doesn\'t support the Fatec Bootstrap Customizer. Please take a second\
